...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      animate={{
        padding: visible ? "18px 24px" : "18px 0px",
  ...
  return (
    <motion.div
      animate={{
        ...
        paddingRight: visible ? "18px" : "2px",
        paddingLeft: visible ? "18px" : "2px",
        borderRadius: visible ? "full" : "2rem",
        y: visible ? 20 : 0,
      }}
      transition={{
        type: "spring",
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-white px-6 py-8 backdrop-blur-[10px] dark:bg-neutral-950/80",
            className,
          )}
        >
          {children}
